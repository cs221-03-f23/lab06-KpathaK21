...
char *scan_binary(struct scan_token_st *tp, char *p, char *end) {
  tp->id = TK_BINLIT;
  int i = 0;
...
char *scan_hexadecimal(struct scan_token_st *tp, char *p, char *end) {
  tp->id = TK_HEXLIT;
  int len = 0;
  bool scan_is_hexlit = true;

  while (scan_is_hexlit) {
    if (p < end && (scan_is_digit(*p) || (*p >= 'A' && *p <= 'F') ||
                    (*p >= 'a' && *p <= 'f'))) {
      tp->name[len] = *p;
      p++;
      len++;
...
char *scan_token(char *p, char *end, struct scan_token_st *tp) {
  if (p == end) {
    ...
    // Hexadecimal literal
    p += 2; // Skip '0x' or '0X'
    p = scan_hexadecimal(tp, p, end);
  } else if (p[0] == '0' && (p[1] == 'b' || p[1] == 'B')) {
    // Binary literal
    p += 2; // Skip '0b' or '0B'
    p = scan_binary(tp, p, end);
